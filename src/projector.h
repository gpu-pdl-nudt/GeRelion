...
public:
	...
	*/
	...
	void computeFourierTransformMap_gpu(MultidimArray<double>& vol_in, MultidimArray<double>& power_spectrum, int current_size = -1, int nr_threads = 1, bool do_gridding = true);
	void griddingCorrect_gpu(double* vol_in, int xdim, int ydim, int zdim, double interpolator, double r_min_nn, int ori_size_padding_factor);
