...
public:
	...
	*/
	...
	//======================================================================//
	//Some functions added for FourierTransform on GPU
public:
	...
	void FourierTransform_gpu(T* V1, T1* V2, int nr_images, int xdim, int ydim, int zdim, bool getCopy = true)
	{
		...
		if (getCopy)
		{
			getFourierCopy_gpu(V2, nr_images * zdim * ydim * (xdim / 2 + 1));
		}
		else
		{
			REPORT_ERROR("GPU Fouriour can not use get alias!!!");
		}

		//else         V2=getFourierAlias_gpu();

		//cudaFree(fFourier_D);

	}
	template <typename T, typename T1>
	void inverseFourierTransform_gpu(T* V1, T1* V2, int nr_images, int xdim, int ydim, int zdim)
	{
		setReal_gpu(V2, nr_images, xdim, ydim, zdim);
		//fFourier_D = V1;
	...
	/*
	{
	    setReal_gpu(V1, V2, nr_images, xdim, ydim, zdim);
	    Transform_gpu(CUFFT_INVERSE);
...
//==================================================================================//
//globle GPU functions, which will call the gpu kernels
