...
extern void compare_CPU_GPU(Complex* in_cpu, cufftDoubleComplex* in_gpu, int size_);
extern void compare_CPU_GPU(double* in_cpu, double* in_gpu, int size_);
template <typename T>
extern void centerFFT_2_gpu(T* in, T* out, int nr_images, int dim, int xdim, int ydim, int zdim, bool forward);
...
__global__ void normalise_model_data_kernel(cufftDoubleComplex* Faux_D, cufftDoubleComplex* gdata_ptr, double* power_spectrum, double* counter, double normfft, int x1dim, int y1dim, int z1dim, int x2dim, int y2dim,  int z2dim,
...
void normalise_model_data_gpu(cufftDoubleComplex* Faux_D, cufftDoubleComplex* gdata_ptr, double* power_spectrum, double* counter, double normfft, int padoridim, int xdim, int ydim, int zdim,  int ref_dim, int max_r2, int padding_factor)
{
...
void Projector::computeFourierTransformMap_gpu(MultidimArray<double>& vol_in, MultidimArray<double>& power_spectrum, int current_size, int nr_threads, bool do_gridding)
{
	...
	cufftDoubleComplex* Faux_D;
	cudaMalloc((void**)&Faux_D, (padoridim / 2 + 1)*padoridim * (ref_dim == 2 ? 1 : padoridim)*sizeof(cufftDoubleComplex));
	...
	cufftDoubleComplex* gdata_ptr;
	cudaMalloc((void**)&gdata_ptr, data.zyxdim * sizeof(cufftDoubleComplex));
	cudaMemset(gdata_ptr, 0., data.zyxdim * sizeof(cufftDoubleComplex));
	...
	cudaMemcpy(data.data, gdata_ptr, sizeof(cufftDoubleComplex)*data.zyxdim, cudaMemcpyDeviceToHost);
...
__global__ void griddingCorrect_kernel(double* vol_in, int xdim, int ydim, int zdim, int xinit, int yinit, int zinit, double interpolator, double r_min_nn, int ori_size_padding_factor)
{
	int global_index = threadIdx.x + blockIdx.x * blockDim.x;
...
void Projector::griddingCorrect_gpu(double* vol_in, int xdim, int ydim, int zdim, double interpolator, double r_min_nn, int ori_size_padding_factor)
{
