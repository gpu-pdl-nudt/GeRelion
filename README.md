...
We extended the original signal subtraction algorithm in GeRelion, named __dynamic signal subtraction (DSS)__. This procedure was originally proposed and tested in Relion (Nature 2015, 523:47). The parameters required for DSS are --red_mask and --yellow_map (or --yellow_mask).
...
__--yellow_mask__ (yellow_mask_file): User-provided mask for the unwanted region. During DSS, GeRelion multiplies the 3D reconstruction from the last iteration with this mask to generate the reference map of the unwanted region, i.e. "yellow map". This yellow map is used to generate 2D projections, which are then subtracted from the original particle images. 

__--yellow_map__ (yellow_map_file): User-provided "yellow map", which will be used in every iteration. When --yellow map is present, --yellow_mask is ignored. (The unwanted region often becomes weaker during iterative refinement, and thus contains not enough signal for subtraction. Therefore, it is often preferred to use a fixed "yellow_map" instead of a "yellow_mask" to generate "yellow_map" during iterations.)

__--extra_iter__ (0): Extra number of iterations to perform. GeRelion auto-refine iterates additional n cycles even after the convergence is achieved.

__--fsc_mask__ (fsc_mask_file): User-provided mask to be applied on the half volumes before FSC calculation.
